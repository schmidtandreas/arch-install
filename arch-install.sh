...
isUserExists() {
	getent passwd $1 1>/dev/null 2>&1 || doErrorExit "User '%s' not exists (called by: %s line: %d)" $1 ${FUNCNAME[1]} ${BASH_LINENO[1]} 
}
...
doInstallYaourt() {
	...
	chown -R $USER_NAME:users ./package-query || doErrorExit "Change owner of package-query failed"
	cd package-query
	doAsUser $USER_NAME makepkg -si --noconfirm --needed || doErrorExit "Install package-query failed"
	cd ..

	git clone $AUR_YAOURT_URL yaourt || doErrorExit "Clone yaourt failed"
	[ ! -d ./yaourt ] && doErrorExit "Clone yaourt failed"
	chown -R $USER_NAME:users ./yaourt || doErrorExit "Change owner of /tmp/yaourt failed"
	cd yaourt
	doAsUser $USER_NAME makepkg -si --noconfirm --needed || doErrorExit "Install yaourt failed"
	cd ..
	
	popd

	if [ -z "$(cat /etc/pacman.conf | grep "\[archlinuxfr\]")" ]; then
...
doSetPassword() {
	...
	while [ $TRIES -lt 3 ]; do
		passwd $PW_USER
		local RET=$?
		[ $RET -eq 0 ] && TRIES=3 || doPrint "Set password failed, try again"
		TRIES=$(($TRIES + 1))
	done
...
doWipeAllPartitions() {
	for i in $(doGetAllPartitions | sort -r); do
		if [ -n "$(mount -l | grep $INSTALL_DEVICE_PATH/$i)" ]; then
			local MOUNT_POINT="$(mount -l | grep $INSTALL_DEVICE_PATH/$i | cut -d ' ' -f 3)"
...
doChroot() {
	...
	arch-chroot /mnt /usr/bin/bash -c "'$IN_CHROOT_SCRIPT_PATH/$SCRIPT_FILE' -c '$IN_CHROOT_CONF_FILE' $1"
	if [ $? != 0 ]; then
...
doCreateSystemDBootEntry() {
	local ENTRY_FILE="/boot/loader/entries/$(cat /etc/machine-id)-$(uname -r).conf"
	local OPTIONS=""
...
doAddUser() {
	...
	if [ -n "$USER_LOCALE" -a -n "$USER_LOCALE_LANG" ]; then
...
doCustomize() {
	if [ $CUSTOMIZE == "yes" ]; then
		...
		[ -n $CUSTOMIZE_TARGET_DIR ] && doUserClodeGitRepo $USER_NAME $CUSTOMIZE_GIT_URL $CUSTOMIZE_TARGET_DIR
